...
const { attachment } = defineProps<{ attachment: ReferenceImagePresentation }>()
const viewerOpen = ref(false)
const viewLabel = `View reference image ${attachment.name}`
</script>
...
<template>
  <HoverCardRoot :open-delay="350" :close-delay="100">
    <HoverCardTrigger as-child>
      <button
        ...
        class="block overflow-hidden rounded-lg border border-white/20 bg-black/15 text-left hover:border-white/40 focus-visible:outline-2 focus-visible:outline-white"
        @click="viewerOpen = true"
      >
        <img :src="attachment.previewURL" :alt="attachment.name" class="h-20 w-28 object-contain" />
        <span class="block max-w-28 truncate border-t border-white/15 px-1.5 py-1 text-[9px]">
          {{ attachment.name }}
        </span>
      </button>
    </HoverCardTrigger>
  ...
  <AppDialogRoot v-model:open="viewerOpen" size="xl">
    <AppDialogHeader
      ...
      description="Reference image preview"
